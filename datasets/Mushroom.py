...
class Mushroom(Dataset):
    ...
    def get_dataset(self, hyper: HyperClass):
        if hyper.dataset == "Mushroom":
            train_dataset = MushroomDataSet(root='/tmp/pycharm_project_400/data/Mushrooms', train=True,
                                            download=True, transform=self.train_transform)

            test_dataset = MushroomDataSet(root='/tmp/pycharm_project_400/data/Mushrooms', train=False,
...
class MushroomDataSet(Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False, split=0.8):
        self.root = root
    ...
    def load_data_dir(self):
        ...
        for folder_name in folder_names:
            ...
            split_index = int(len(images_in_folder) * self.split)
            if self.train:
                images_in_folder = images_in_folder[:split_index]
            else:
                images_in_folder = images_in_folder[split_index:]
...
if __name__ == '__main__':
    dataset = MushroomDataSet(root='/tmp/pycharm_project_400/data/Mushrooms', download=True)
